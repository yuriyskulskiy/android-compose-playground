...
fun Modifier.alphaGaussianBlurTest101(
    radius: Dp,
    ...
    val effect = rememberAlphaGaussianBlurEffect101(radiusPx = rPx, color = color)
...
private const val AlphaGaussianBlur101TapAglsl = """
uniform shader src;
uniform float2 uDir;
...
half4 main(float2 p) {
    ...
    float stepPx = max(0.125, uRadius / 50.0);
...
private const val AlphaGaussianBlur101TapTintAglsl = """
uniform shader src;
uniform float2 uDir;
uniform float  uRadius; // px
...
@Composable
private fun rememberAlphaGaussianBlurEffect101(
    radiusPx: Float,
    color: Color,
): RenderEffect {
    ...
    return remember(r, color) {
        val rtH = RuntimeShader(AlphaGaussianBlur101TapAglsl).apply {
            setFloatUniform("uDir", 1f, 0f)
            setFloatUniform("uRadius", r)
        }
        val rtV = RuntimeShader(AlphaGaussianBlur101TapTintAglsl).apply {
